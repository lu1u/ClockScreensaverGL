...
namespace ClockScreenSaverGL.DisplayedObjects.Fonds.TroisD
{
    ...
    public class Nebuleuse : TroisD, IDisposable
    {
        ...
        private static float RATIO_VITESSE_NUAGES = c.getParametre("Ratio Vitesse Nuages", 1.5f, (a) => { RATIO_VITESSE_NUAGES = (float)Convert.ToDouble(a); } );
        ...
        private static float VITESSE_ROTATION = c.getParametre("VitesseRotation", 10f, (a) => { VITESSE_ROTATION = (float)Convert.ToDouble(a); } );
        private static float VITESSE_TRANSLATION = c.getParametre("VitesseTranslation", 0.1f, (a) => { VITESSE_TRANSLATION = (float)Convert.ToDouble(a); } );
        private static float VITESSE = c.getParametre("Vitesse", 5.0f, (a) => { VITESSE = (float)Convert.ToDouble(a); } );
        private static float DELTA_COULEUR = c.getParametre("Delta Couleur", 0.1f, (a) => { DELTA_COULEUR = (float)Convert.ToDouble(a); } );
        private static bool ADDITIVE = c.getParametre("Additive", false, (a) => { ADDITIVE = Convert.ToBoolean(a); } );
        private static byte ALPHA_ETOILE = c.getParametre("Alpha Etoile", (byte)255, (a) => { ALPHA_ETOILE = Convert.ToByte(a); } );
        private static byte ALPHA_NUAGE = c.getParametre("Alpha Nuage", (byte)64, (a) => { ALPHA_NUAGE = Convert.ToByte(a); }   );
        private static float TAILLE_ETOILE = c.getParametre("Taille Etoiles", 0.25f, (a) => { TAILLE_ETOILE = (float)Convert.ToDouble(a); } );
        private static float TAILLE_NUAGE = c.getParametre("Taille Nuages", 10.0f, (a) => { TAILLE_NUAGE = (float)Convert.ToDouble(a); } );
