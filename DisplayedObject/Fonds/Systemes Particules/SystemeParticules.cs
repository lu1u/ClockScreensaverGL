...
{
    class SystemeParticules : Fond, IDisposable
    {
        ...
        private List<Emetteur> _listeEmetteurs = new List<Emetteur>();
        ...
        public SystemeParticules(OpenGL gl, int NbMaxParticules): base(gl)
        {
            NB_MAX_PARTICULES = NbMaxParticules;
            _particules = new Particule[NB_MAX_PARTICULES];
            for (int i = 0; i < NB_MAX_PARTICULES; i++)
                _particules[i] = new Particule();
        ...
        public void AjouteEmetteur(Emetteur e)
        {
        ...
        {
            _trier = false;
            foreach (Emetteur e in _listeEmetteurs)
