...
namespace ClockScreenSaverGL.DisplayedObjects.Fonds.Particules
{
    class AttracteurParticules : SystemeParticules, IDisposable
    {
        ...
        readonly float ALPHA_MODIFIEUR = conf.getParametre(CAT, "Modifieur Alpha", 0.005f);
        readonly float TAILLE_PARTICULE = conf.getParametre(CAT, "TailleParticule", 0.012f);
        readonly float VITESSE_PARTICULE = conf.getParametre(CAT, "VitesseParticule", 0.5f);
        ...
        public AttracteurParticules(OpenGL gl) : base(gl, NB_PARTICULES)
        {
            ...
            {
                //TrajectoireDiagonale t = new TrajectoireDiagonale(FloatRandom(-0.4f, -0.2f), FloatRandom(-0.4f, 0.4f), 0.02f * SigneRandom(), 0.04f * SigneRandom());
                Trajectoire t = new TrajectoireOvale(0, 0, MAX_X*0.8f, MAX_Y*0.8f, VITESSE_EMETTEUR * FloatRandom(0.5f, 1.5f), -(float)Math.PI/2.0f);
                AjouteEmetteur(new EmetteurJet(TAILLE_PARTICULE, VITESSE_PARTICULE, 20, t));
            }
            ...
            {
                //TrajectoireDiagonale t = new TrajectoireDiagonale(FloatRandom(0.2f, 0.7f), FloatRandom(-0.7f, 0.7f), FloatRandom(-0.01f, 0.01f), FloatRandom(-0.01f, 0.01f));
