...
{
    class EmetteurFeuArtifice: Emetteur
    {
        ...
        public override void Deplace(SystemeParticules s, Temps maintenant, Color couleur)
        {
            ...
            if (_timer.Ecoule())
            {
                float X = DisplayedObject.FloatRandom(SystemeParticules.MIN_X, SystemeParticules.MAX_X);
                float Y = DisplayedObject.FloatRandom(SystemeParticules.MIN_Y, SystemeParticules.MAX_Y);
