...
namespace ClockScreenSaverGL.DisplayedObjects.Fonds.TroisD.Opengl
{
    ...
    public class NuagesOpenGL : TroisD, IDisposable
    {
        ...
        static readonly float ALPHA = 0.004f;// conf.getParametre(CAT, "Alpha", 0.25f);
        ...
        static readonly int NB_NUAGES = 20;// conf.getParametre(CAT, "Nb", 10);
        static readonly int MAX_NIVEAU = 6;// conf.getParametre(CAT, "NbNiveaux", 6);
        ...
        void creerNuage(uint i, bool init)
        {
            ...
            _nuages[i].y = FloatRandom(0, VIEWPORT_Y) * 2.0f * _positionNuage;
        ...
        public override void Deplace(Temps maintenant, Rectangle tailleEcran)
        {
            ...
            for (int i = 0; i < NB_NUAGES; i++)
            {
...

                {
                    ...
                    _nuages[i].x = FloatRandom(-VIEWPORT_X, VIEWPORT_X) * 10.0f;
                    _nuages[i].y = FloatRandom(0, VIEWPORT_Y) * _positionNuage;
                    trierNuages = true;
                }
            }
